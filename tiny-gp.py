...
# evolved tree presented in text format
# to also create a nice image install https://pypi.org/project/graphviz/
try:
...
class GPTree:
    ...
    def draw_tree(self, footer):
        if GRAPH:
            dot = [Digraph()]
            dot[0].attr(kw='graph', label=footer)
            count = [0]
            self.draw(dot, count)
            dot[0].render('GPTree', view = True, cleanup = True)  
